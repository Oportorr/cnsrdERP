...
class Invoices_model extends CI_Model
{
     ...
     * Start
     * Author : AT
     * File Name: Sales_model.php
     * Description: We create a new getLgnNumber function for used to genrate a LGN Number for sales order. this code is follow the proper CI Structure.Before this created function, this lgn number genrated code in Sales controller and follow the core mysql_query function. so i have correct this issue by proper CI structure.
     * End
     */

    public function getLgnData($data) {

        
        $this->db->select('lgn.starting_seq,lgn.id,lgn.lgn_due_date, lgn.ending_seq, lgn.counter, lgn.lgn_type, lgn.series, lgn.business_division, lgn.emission_pt,lgn.printer_area,lgn.overide_seq');
        $this->db->from('geopos_lgn AS lgn');
        $this->db->join('geopos_customers AS com', 'com.lgn_type = lgn.lgn_type');
        $this->db->where(array('com.id' => $data['customer_id'], 'lgn.status' => 1));
        $query = $this->db->get();
        $result = $query->row_array();
        
        //$lgn_count_row = $this->getCustomerSales(array("customer_id" => $data['customer_id']));

        //$lgn_count_row = $this->getCustomerSalesNew($data);
            
        $st = $result['starting_seq'];
        $et = $result['ending_seq'];
   
        $total_count = count(range($result['starting_seq'],$result['ending_seq']));

        //$cal = $lgn_count_row % $total_count;
        $cal  = $result['counter'];

        $lgnpoint = '';
        if($result['overide_seq'] == 1) {
            $lgnpoint = str_pad($st  + $cal, strlen($st), '0', STR_PAD_LEFT);
        } else {
            if($result['counter'] < $total_count) {
                $lgnpoint = str_pad($st  + $cal, strlen($st), '0', STR_PAD_LEFT);
            }
        }
        ...
        if($result['id']){
            ...
            $this->db->update('geopos_lgn');
        }
        

        $lgn_number = $result['series'] . $result['lgn_type'] . $lgnpoint;  

        return array(
            'lgn_number'=>$lgn_number,
            'lgn_due_date'=>$result['lgn_due_date']
        );
    }

    public function getLgnDataOld($data) {

        $this->db->select('lgn.starting_seq,lgn.lgn_due_date, lgn.ending_seq, lgn.lgn_type, lgn.series, lgn.business_division, lgn.emission_pt,lgn.printer_area,lgn.overide_seq');
        $this->db->from('geopos_lgn AS lgn');
        $this->db->join('geopos_customers AS com', 'com.lgn_type = lgn.lgn_type');
        $this->db->where(array('com.id' => $data['customer_id'], 'lgn.status' => 1));
        $query = $this->db->get();
        ...
        //$lgn_count_row = $this->getCustomerSalesNew($data);
        $lgn_count_row = $this->getCustomerSalesNew();
     ...
     * Author : AT
     * File Name: Sales_model.php
     * Description: we create a getCustomerSales function for get a customer id.
     */

    // SELECT lgn.starting_seq, lgn.series, lgn.business_division, lgn.emission_pt,lgn.printer_area FROM `geopos_lgn` lgn left join geopos_customers com on com.lgn_type = lgn.`lgn_type` WHERE com.id = 17 AND lgn.`status` = 1

    public function getCustomerSales($data) {
        $query = $this->db->get_where('geopos_invoices', array("csd" => $data['customer_id']));
        return $row = $query->num_rows();
    }

    public function getCustomerSalesNew() 
    
    /*{

        $this->db->from('geopos_invoices');
        $this->db->where("status != 'canceled'");
        $this->db->where("csd", $data['customer_id']);
        $query = $this->db->get();
        return $row = $query->num_rows();
    }*/
    {
        $query = $this->db->get_where('geopos_invoices',array("status != 'canceled'"));
        return $row = $query->num_rows();
    }
}